...
const Hero = () => {
    ...
    const [emergencyIssues, setEmergencyIssues] = useState(0);
  
    useEffect(() => {
      // Fetch data from your backend API
      fetch('http://localhost:4000/api/getreports')
        .then((response) => response.json())
        .then((data) => {
            console.log('API response data:', data);

         // Check if 'data' property exists in the response
      const reportsArray = data.data || [];

      // Calculate counts based on data
      const total = reportsArray.length || 0;
      const solved = reportsArray.filter((report) => report.status === 'not solved').length || 0;
      const emergency = reportsArray.filter((report) => report.emergencyStatus === 'emergency').length || 0;

          // Set state variables
          setTotalIssues(total);
          setSolvedIssues(solved);
          setEmergencyIssues(emergency);
        })
        .catch((error) => {
            ...
            <ol className = "inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div className = "flex items-center">
                        ...
                        <a href="#" className = "ml-1 text-sm font-medium text-gray-700 hover:text-gray-900 md:ml-2 dark:text-gray-400 dark:hover:text-white">Dashboard</a>
                    </div>
                </li>
            </ol>
        </nav>
